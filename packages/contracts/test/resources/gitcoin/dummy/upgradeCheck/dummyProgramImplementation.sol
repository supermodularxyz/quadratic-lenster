...
contract DummyProgramImplementation is AccessControlEnumerable, Initializable {
     ...
     */
    function initialize(bytes calldata encodedParameters, string calldata newFoobar) external initializer {
        foobar = newFoobar;

        // Decode _encodedParameters
        (MetaPtr memory _metaPtr, address[] memory _adminRoles, address[] memory _programOperators) = abi.decode(
            encodedParameters,
